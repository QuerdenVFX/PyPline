...
frame1.place(height=200, width=800, relx=.025)
...
remove_bt = tk.Button(edit_frame, text="-", height=1 ,width=2, command=lambda: remove())
...
def add():
    add_info = [soft_en.get(), os.path.abspath(path_en.get()), folders_en.get(), ext_en.get()]
...
def update():
    ...
    if not selected_items:
        tk.messagebox.showwarning("Avertissement", "Veuillez sélectionner une ligne à mettre à jour.")
        return

    selected_item = selected_items[0]  # Récupère l'élément sélectionné
    new_values = [soft_en.get(), path_en.get(), folders_en.get(), ext_en.get()]  # Récupère les nouvelles valeurs des champs d'entrée
